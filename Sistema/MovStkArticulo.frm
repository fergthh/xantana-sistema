...
Begin VB.Form PrgMovStkArticulo 
   AutoRedraw      =   -1  'True
   ...
   Begin VB.CommandButton Ultimo 
      ...
      Height          =   975
      Left            =   11400
      MouseIcon       =   "MovStkArticulo.frx":0000
      MousePointer    =   99  'Custom
      Picture         =   "MovStkArticulo.frx":030A
      Style           =   1  'Graphical
      TabIndex        =   24
      ToolTipText     =   "Salida"
      Top             =   5400
   ...
   Begin VB.CommandButton Graba 
      ...
      Height          =   975
      Left            =   11400
      MouseIcon       =   "MovStkArticulo.frx":074C
      MousePointer    =   99  'Custom
      Picture         =   "MovStkArticulo.frx":0A56
      Style           =   1  'Graphical
      TabIndex        =   21
   ...
   Begin VB.CommandButton Limpia 
      ...
      Height          =   975
      Left            =   11400
      MouseIcon       =   "MovStkArticulo.frx":1298
      MousePointer    =   99  'Custom
      Picture         =   "MovStkArticulo.frx":15A2
      Style           =   1  'Graphical
      TabIndex        =   20
   ...
   Begin VB.CommandButton Consulta 
      ...
      Height          =   975
      Left            =   11400
      MouseIcon       =   "MovStkArticulo.frx":1DE4
      MousePointer    =   99  'Custom
      Picture         =   "MovStkArticulo.frx":20EE
      Style           =   1  'Graphical
      TabIndex        =   19
   ...
   Begin VB.CommandButton CmdClose 
      ...
      Height          =   975
      Left            =   11400
      MouseIcon       =   "MovStkArticulo.frx":2930
      MousePointer    =   99  'Custom
      Picture         =   "MovStkArticulo.frx":2C3A
      Style           =   1  'Graphical
      TabIndex        =   18
   ...
   Begin VB.ListBox Pantalla 
      ...
      EndProperty
      Height          =   1740
      ItemData        =   "MovStkArticulo.frx":347C
      Left            =   120
      List            =   "MovStkArticulo.frx":3483
      TabIndex        =   1
      Top             =   6240
      Visible         =   0   'False
      Width           =   6855
   ...
   End
End
Attribute VB_Name = "PrgMovStkArticulo"
...
Private Sub Graba_Click()
    ...
    If Val(Numero.Text) = 0 Then
        ...
        ZSql = ZSql + " FROM MovStkArticulo"
        spMovStkArticulo = ZSql
        Set rstMovStkArticulo = db.OpenRecordset(spMovStkArticulo, dbOpenSnapshot, dbSQLPassThrough)
        If rstMovStkArticulo.RecordCount > 0 Then
            rstMovStkArticulo.MoveLast
            ZUltimo = IIf(IsNull(rstMovStkArticulo!NumeroMayor), "0", rstMovStkArticulo!NumeroMayor)
            Numero.Text = ZUltimo + 1
            rstMovStkArticulo.Close
        End If
    ...
    For WRenglon = 1 To 100
        ...
        ZSql = ZSql + "Select *"
        ZSql = ZSql + " FROM MovStkArticulo"
        ZSql = ZSql + " Where MovStkArticulo.Clave = " + "'" + WClave + "'"
        spMovStkArticulo = ZSql
        Set rstMovStkArticulo = db.OpenRecordset(spMovStkArticulo, dbOpenSnapshot, dbSQLPassThrough)
        If rstMovStkArticulo.RecordCount > 0 Then
            
            WArticulo = rstMovStkArticulo!Articulo
            WCantidad = Str$(rstMovStkArticulo!Cantidad)
            ZDeposito = IIf(IsNull(rstMovStkArticulo!Deposito), "0", rstMovStkArticulo!Deposito)
            ZDepositoII = IIf(IsNull(rstMovStkArticulo!DepositoII), "0", rstMovStkArticulo!DepositoII)
            
            If ZDepositoII > 0 Then
                WCantidad = Str$(Val(WCantidad) * -1)
            End If
            ...
            Select Case ZDeposito
                ...
                Case 2
                    ZSql = ""
                    ZSql = ZSql + "UPDATE Articulo SET "
                    ZSql = ZSql + " Stock = Stock  - " + "'" + WWCantidad + "',"
                    ZSql = ZSql + " StockIII = StockIII  - " + "'" + WWCantidad + "'"
                ...
                Case 3
                    ...
                    ZSql = ZSql + "UPDATE Articulo SET "
                    ZSql = ZSql + " Stock = Stock  - " + "'" + WCantidad + "',"
                    ZSql = ZSql + " StockIV = StockIV  - " + "'" + WCantidad + "'"
                    ZSql = ZSql + " Where Codigo = " + "'" + WArticulo + "'"
                    spArticulo = ZSql
                    Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
                Case 4
                    ...
                    ZSql = ZSql + "UPDATE Articulo SET "
                    ZSql = ZSql + " Stock = Stock  - " + "'" + WCantidad + "',"
                    ZSql = ZSql + " StockV = StockV  - " + "'" + WCantidad + "'"
                    ZSql = ZSql + " Where Codigo = " + "'" + WArticulo + "'"
                    spArticulo = ZSql
                    Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
                Case 5
                    ...
                    ZSql = ZSql + "UPDATE Articulo SET "
                    ZSql = ZSql + " Stock = Stock  - " + "'" + WCantidad + "',"
                    ZSql = ZSql + " StockVI = StockVI  - " + "'" + WCantidad + "'"
                    ZSql = ZSql + " Where Codigo = " + "'" + WArticulo + "'"
                    spArticulo = ZSql
                    Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
                Case Else
            ...
            Select Case ZDepositoII
                ...
                Case 3
                    ...
                    ZSql = ZSql + "UPDATE Articulo SET "
                    ZSql = ZSql + " Stock = Stock  + " + "'" + WCantidad + "',"
                    ZSql = ZSql + " StockIII = StockIII  + " + "'" + WCantidad + "'"
                    ZSql = ZSql + " Where Codigo = " + "'" + WArticulo + "'"
                    spArticulo = ZSql
                    Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
                Case 4
    ...
    ZSql = ""
    ZSql = ZSql + "DELETE MovStkArticulo"
    ZSql = ZSql + " Where MovStkArticulo.Numero = " + "'" + Numero.Text + "'"
    spMovStkArticulo = ZSql
    Set rstMovStkArticulo = db.OpenRecordset(spMovStkArticulo, dbOpenSnapshot, dbSQLPassThrough)
    ...
    For IRow = 1 To 100
        ...
        If Cantidad <> 0 Then
            ...
            Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
            If rstArticulo.RecordCount > 0 Then
                ZZStock = Str$(rstArticulo!Stock)
                rstArticulo.Close
            End If
            
            
            ZSql = ""
            ZSql = ZSql + "INSERT INTO MovStkArticulo ("
            ZSql = ZSql + "Clave ,"
            ZSql = ZSql + "Numero ,"
            ZSql = ZSql + "Renglon ,"
            ZSql = ZSql + "fecha ,"
...
Private Sub Impresion()
    ...
    Listado.SQLQuery = "SELECT MovStkArticulo.Numero, MovStkArticulo.Renglon, MovStkArticulo.fecha, MovStkArticulo.Articulo, MovStkArticulo.Cantidad, MovStkArticulo.Observaciones, MovStkArticulo.Stock, " _
            + "Articulo.Descripcion, Articulo.Stock " _
            + "From " _
            + DSQ + ".dbo.MovStkArticulo MovStkArticulo, " _
            + DSQ + ".dbo.Articulo Articulo " _
            + "Where " _
            + "MovStkArticulo.Articulo = Articulo.Codigo AND " _
            + "MovStkArticulo.Numero >= " + Numero.Text + " AND " _
            + "MovStkArticulo.Numero <= " + Numero.Text
    
    Listado.Connect = Connect()
    
    Uno = "{MovStkArticulo.Numero} in " + Numero.Text + " to " + Numero.Text
    
    Listado.GroupSelectionFormula = Uno
    Listado.SelectionFormula = Uno
            
    Listado.ReportFileName = "ImpreMovStkArticulo.rpt"
...
Private Sub Limpia_Click()
    ...
    Rem ZSql = ZSql + " FROM MovStkArticulo"
    Rem spMovStkArticulo = ZSql
    Rem Set rstPRODUCCION = db.OpenRecordset(spMovStkArticulo, dbOpenSnapshot, dbSQLPassThrough)
...
Private Sub Proceso_Click()
    ...
    For WRenglon = 1 To 100
        ...
        If rstMovStkArticulo.RecordCount > 0 Then
            ...
            WVector1.Col = 1
            WVector1.Text = rstMovStkArticulo!Articulo
            Auxi1 = rstMovStkArticulo!Articulo
                
            WVector1.Col = 3
            WVector1.Text = Pusing("###,###", Str$(rstMovStkArticulo!Cantidad))
            
            WStkAnt = IIf(IsNull(rstMovStkArticulo!stkant), "0", rstMovStkArticulo!stkant)
            WStkAct = IIf(IsNull(rstMovStkArticulo!Stkact), "0", rstMovStkArticulo!Stkact)
...
Private Sub Numero_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        ZSql = ZSql + "Select *"
        ZSql = ZSql + " FROM MovStkArticulo"
        ZSql = ZSql + " Where MovStkArticulo.Numero = " + "'" + Auxi + "'"
        spMovStkArticulo = ZSql
        Set rstMovStkArticulo = db.OpenRecordset(spMovStkArticulo, dbOpenSnapshot, dbSQLPassThrough)
        If rstMovStkArticulo.RecordCount > 0 Then
            
            Fecha.Text = rstMovStkArticulo!Fecha
            Observaciones.Text = rstMovStkArticulo!Observaciones
            ZDeposito = IIf(IsNull(rstMovStkArticulo!Deposito), "0", rstMovStkArticulo!Deposito)
            Deposito.ListIndex = ZDeposito
            ZDepositoII = IIf(IsNull(rstMovStkArticulo!DepositoII), "0", rstMovStkArticulo!DepositoII)
            DepositoII.ListIndex = ZDepositoII
            Concepto.Text = IIf(IsNull(rstMovStkArticulo!Concepto), "", rstMovStkArticulo!Concepto)
...
Private Sub Ultimo_Click()
    ...
    If rstMovStkArticulo.RecordCount > 0 Then
        rstMovStkArticulo.MoveLast
        Numero.Text = IIf(IsNull(rstMovStkArticulo!NumeroMayor), "0", rstMovStkArticulo!NumeroMayor)
        rstMovStkArticulo.Close
