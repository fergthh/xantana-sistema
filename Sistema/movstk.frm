...
Begin VB.Form PrgMovStk 
   ...
   Begin VB.CommandButton Ultimo 
      ...
      Height          =   975
      Left            =   10440
      MouseIcon       =   "movstk.frx":0000
      MousePointer    =   99  'Custom
      Picture         =   "movstk.frx":030A
      Style           =   1  'Graphical
      TabIndex        =   26
      ToolTipText     =   "Salida"
      Top             =   5520
   ...
   Begin VB.CommandButton Graba 
      ...
      Height          =   975
      Left            =   10440
      MouseIcon       =   "movstk.frx":074C
      MousePointer    =   99  'Custom
      Picture         =   "movstk.frx":0A56
      Style           =   1  'Graphical
      TabIndex        =   21
   ...
   Begin VB.CommandButton Limpia 
      ...
      Height          =   975
      Left            =   10440
      MouseIcon       =   "movstk.frx":1298
      MousePointer    =   99  'Custom
      Picture         =   "movstk.frx":15A2
      Style           =   1  'Graphical
      TabIndex        =   20
   ...
   Begin VB.CommandButton Consulta 
      ...
      Height          =   975
      Left            =   10440
      MouseIcon       =   "movstk.frx":1DE4
      MousePointer    =   99  'Custom
      Picture         =   "movstk.frx":20EE
      Style           =   1  'Graphical
      TabIndex        =   19
   ...
   Begin VB.CommandButton CmdClose 
      ...
      Height          =   975
      Left            =   10440
      MouseIcon       =   "movstk.frx":2930
      MousePointer    =   99  'Custom
      Picture         =   "movstk.frx":2C3A
      Style           =   1  'Graphical
      TabIndex        =   18
   ...
   Begin VB.ListBox Pantalla 
      ...
      EndProperty
      Height          =   1740
      ItemData        =   "movstk.frx":347C
      Left            =   120
      List            =   "movstk.frx":3483
      TabIndex        =   1
      Top             =   6240
      Visible         =   0   'False
      Width           =   6855
...
Private Sub Graba_Click()
    ...
    If Deposito.ListIndex <> 1 And Deposito.ListIndex <> 2 Then
        m$ = "Se debe informar el deposito en el que se realiza la operacion"
    ...
    If Val(Numero.Text) = 0 Then
        ...
        If rstMovstk.RecordCount > 0 Then
            rstMovstk.MoveLast
            ZUltimo = IIf(IsNull(rstMovstk!NumeroMayor), "0", rstMovstk!NumeroMayor)
    ...
    For WRenglon = 1 To 100
        ...
        ZSql = ZSql + "Select *"
        ZSql = ZSql + " FROM MovStk"
        ZSql = ZSql + " Where MovStk.Clave = " + "'" + WClave + "'"
        spMovStk = ZSql
        Set rstMovstk = db.OpenRecordset(spMovStk, dbOpenSnapshot, dbSQLPassThrough)
        If rstMovstk.RecordCount > 0 Then
            
            WArticulo = rstMovstk!articulo
            WCantidad = rstMovstk!Cantidad
            WDeposito = IIf(IsNull(rstMovstk!Deposito), "1", rstMovstk!Deposito)
            ...
            If WCantidad > 0 Then
                ...
                If WDeposito = 1 Then
                    ...
                    ZSql = ZSql + " Entradas = Entradas - " + "'" + Str$(WCantidad) + "',"
                    ZSql = ZSql + " Stock = Stock - " + "'" + Str$(WCantidad) + "',"
                    ZSql = ZSql + " StockI = StockI - " + "'" + Str$(WCantidad) + "'"
                ...
                If WDeposito = 1 Then
                    ZSql = ""
                    ZSql = ZSql + "UPDATE Articulo SET "
                    ZSql = ZSql + " Salidas = Salidas - " + "'" + Str$(WCantidad * -1) + "',"
                    ZSql = ZSql + " Stock = Stock + " + "'" + Str$(WCantidad * -1) + "',"
                    ZSql = ZSql + " StockI = StockI + " + "'" + Str$(WCantidad * -1) + "'"
                    ZSql = ZSql + " Where Codigo = " + "'" + WArticulo + "'"
                    spArticulo = ZSql
                    Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
                        Else
    ...
    ZSql = ""
    ZSql = ZSql + "DELETE MovStk"
    ZSql = ZSql + " Where MovStk.Numero = " + "'" + Numero.Text + "'"
    spMovStk = ZSql
    Set rstMovstk = db.OpenRecordset(spMovStk, dbOpenSnapshot, dbSQLPassThrough)
        ...
        If Cantidad <> 0 Then
            ...
            If Val(ZZCantidad) > 0 Then
                ...
                If Deposito.ListIndex = 1 Then
                    ...
                    ZSql = ZSql + " Salidas = Salidas + " + "'" + Str$(Val(ZZCantidad) * -1) + "',"
                    ZSql = ZSql + " Stock = Stock - " + "'" + Str$(Val(ZZCantidad) * -1) + "',"
                    ZSql = ZSql + " StockI = StockI - " + "'" + Str$(Val(ZZCantidad) * -1) + "'"
                    ZSql = ZSql + " Where Codigo = " + "'" + ZZArticulo + "'"
                    spArticulo = ZSql
                    Set rstArticulo = db.OpenRecordset(spArticulo, dbOpenSnapshot, dbSQLPassThrough)
                        Else
    ...
    If Val(Carpeta.Text) <> 0 Then
        ...
        ZSql = ZSql + " Estado = 1"
        ZSql = ZSql + " Where Numero = " + "'" + Carpeta.Text + "'"
...
Private Sub Impresion()
    ...
    Listado.SQLQuery = "SELECT Movstk.Numero, Movstk.Renglon, Movstk.fecha, Movstk.Articulo, Movstk.Cantidad, Movstk.Observaciones, Movstk.Stock, " _
            ...
            + "MovStk.Numero >= " + Numero.Text + " AND " _
            + "MovStk.Numero <= " + Numero.Text
    
    Listado.Connect = Connect()
    
    Uno = "{MovStk.Numero} in " + Numero.Text + " to " + Numero.Text
    
    Listado.GroupSelectionFormula = Uno
    Listado.SelectionFormula = Uno
            
    Listado.ReportFileName = "ImpreMovStk.rpt"
...
Private Sub Limpia_Click()
    ...
    Rem ZSql = ZSql + " FROM MovStk"
    Rem spMovStk = ZSql
    Rem Set rstMovstk = db.OpenRecordset(spMovStk, dbOpenSnapshot, dbSQLPassThrough)
    Rem If rstMovstk.RecordCount > 0 Then
    Rem     rstMovstk.MoveLast
    Rem     ZUltimo = IIf(IsNull(rstMovstk!NumeroMayor), "0", rstMovstk!NumeroMayor)
...
Private Sub Form_Load()
    ...
    Deposito.AddItem ""
    Deposito.AddItem "Sarmiento"
    Deposito.AddItem "Elaion"
...
Private Sub Proceso_Click()
    ...
    For WRenglon = 1 To 100
        ...
        If rstMovstk.RecordCount > 0 Then
            ...
            WVector1.Col = 1
            WVector1.Text = rstMovstk!articulo
            Auxi1 = rstMovstk!articulo
                
            WVector1.Col = 3
            WVector1.Text = Pusing("###,###", Str$(rstMovstk!Cantidad))
            
            WStkAnt = IIf(IsNull(rstMovstk!stkant), "0", rstMovstk!stkant)
            WStkAct = IIf(IsNull(rstMovstk!Stkact), "0", rstMovstk!Stkact)
...
Private Sub Numero_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        ZSql = ZSql + "Select *"
        ZSql = ZSql + " FROM MovStk"
        ZSql = ZSql + " Where MovStk.Numero = " + "'" + Auxi + "'"
        spMovStk = ZSql
        Set rstMovstk = db.OpenRecordset(spMovStk, dbOpenSnapshot, dbSQLPassThrough)
        If rstMovstk.RecordCount > 0 Then
            
            Fecha.Text = rstMovstk!Fecha
            Observaciones.Text = rstMovstk!Observaciones
            Carpeta.Text = IIf(IsNull(rstMovstk!Carpeta), "", rstMovstk!Carpeta)
            Deposito.ListIndex = IIf(IsNull(rstMovstk!Deposito), "1", rstMovstk!Deposito)
            rstMovstk.Close
...
Private Sub Carpeta_Keypress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        ...
        If Val(Carpeta.Text) <> 0 Then
            ...
            ZSql = ZSql + " Where OrdenImportacion.Numero = " + "'" + Carpeta.Text + "'"
            spOrdenImportacion = ZSql
            Set rstOrdenImportacion = db.OpenRecordset(spOrdenImportacion, dbOpenSnapshot, dbSQLPassThrough)
            If rstOrdenImportacion.RecordCount > 0 Then
                ZEstado = IIf(IsNull(rstOrdenImportacion!Estado), "0", rstOrdenImportacion!Estado)
                If ZEstado = 1 Then
                    m$ = "Numero de Carpeta ya utilizada"
                    aaaaaa% = MsgBox(m$, 0, "Ingreso y Egreso de Stock")
                End If
...
Private Sub Ultimo_Click()
    ...
    If rstMovstk.RecordCount > 0 Then
        rstMovstk.MoveLast
        Numero.Text = IIf(IsNull(rstMovstk!NumeroMayor), "0", rstMovstk!NumeroMayor)
        rstMovstk.Close
